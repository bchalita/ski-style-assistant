...
const CATALOG_FILES: Array<{ shop: ShopId; path: string }> = [
  {
    shop: "alpineMart",
    path: path.resolve(__dirname, "../../data/alpine_outfitters.csv"),
  },
  {
    shop: "snowBase",
    path: path.resolve(__dirname, "../../data/glacier_gear.csv"),
  },
  {
    shop: "peakShop",
    path: path.resolve(__dirname, "../../data/summit_supply.csv"),
  },
];
...
const buildCatalogForShop = (
  ...
  for (const row of dataRows) {
    ...
    if (!grouped.has(key)) {
      ...
      if (image) {
        attributes.image = image.startsWith("http")
          ? image
          : path.join("data", image);
      }
